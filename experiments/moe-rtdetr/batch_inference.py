...
def _import_modules():
    """延迟导入模块"""
    _setup_paths()
    # [CHANGE] 使用 AdaptiveExpertTrainer
    from train import AdaptiveExpertTrainer, create_backbone
    ...
    return AdaptiveExpertTrainer, create_backbone, DetDETRPostProcessor, BoxProcessFormat
...
def load_model(config_path: str, checkpoint_path: str, device: str = "cuda"):
    """加载模型和权重"""
    # [CHANGE] 导入 AdaptiveExpertTrainer
    AdaptiveExpertTrainer, _, DetDETRPostProcessor, BoxProcessFormat = _import_modules()
    ...
    # 创建训练器以构建模型
    trainer = AdaptiveExpertTrainer(config)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MoE-RT-DETR 批量推理脚本")
