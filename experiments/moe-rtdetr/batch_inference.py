...
def load_model(config_path: str, checkpoint_path: str, device: str = "cuda"):
    """加载模型和权重"""
    AdaptiveExpertRTDETR, DetDETRPostProcessor, BoxProcessFormat = _import_modules()
...
def preprocess_image(image_path: str, target_size: int = 640):
    ...
    # ⚠️ 关键修复：训练时图像未归一化！
    # 训练时：padded_image是float类型，值在[0, 255]范围（从uint8转换来的）
    # 训练时transforms=None，所以没有归一化步骤
    # 为了与训练时完全一致，推理时也不应该归一化
    # 但需要确认模型backbone是否内部处理了归一化
    # 根据检查，backbone没有归一化层，所以应该保持[0, 255]范围
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MOE-RTDETR 批量推理脚本")
