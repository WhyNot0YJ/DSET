    ...
    def __init__(self, cfg_path: str, **kwargs) -> None:
        ...
        if 'training' in cfg:
            ...
            if 'epochs' in training_cfg:
                self.epoches = training_cfg['epochs']
            ...
            if 'save_interval' in training_cfg:
                self.save_interval = training_cfg['save_interval']
            if 'log_dir' in training_cfg:
                self.log_dir = training_cfg['log_dir']
            if 'save_dir' in training_cfg:
                self.save_dir = training_cfg['save_dir']
            if 'ema_decay' in training_cfg:
                self.ema_decay = training_cfg['ema_decay']
            
            # 设置训练相关属性
            self.print_freq = training_cfg.get('print_freq', 10)
            self.clip_max_norm = training_cfg.get('clip_max_norm', 0.1)
            self.checkpoint_freq = training_cfg.get('checkpoint_freq', 10)
