...
class YOLOv10Trainer:
    ...
    def __init__(self, config: Dict, config_path: Optional[str] = None):
        ...
        self.logger.info(f"✓ 初始化YOLOv10训练器")
    ...
    def setup_logging(self):
        ...
        if resume_checkpoint and Path(resume_checkpoint).exists():
            ...
            model_name = self.config.get('model', {}).get('model_name', 'yolov10n')
            # 去掉.pt后缀（如果存在）
            if model_name.endswith('.pt'):
                model_name = model_name[:-3]
            self.experiment_name = f"yolo_{model_name.replace('yolov8', 'v8').replace('yolov10', 'v10').replace('yolo11', 'v11')}"
    ...
    def create_model(self):
        """创建YOLO模型"""
        model_name = self.model_config.get('model_name', 'yolov10n.pt')
...
def main():
    """主函数"""
    parser = argparse.ArgumentParser(description='YOLOv10训练脚本')
    ...
    # 创建训练器
    trainer = YOLOv10Trainer(config, config_path=args.config)
